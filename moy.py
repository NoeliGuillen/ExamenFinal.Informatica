...
# Barrera de entrada
poste_entrada = cylinder(pos=vector(1, 0.1, -12), axis=vector(0, 2.5, 0), radius=0.2, color=color.white)
pivot_barrera_entrada = vector(1, 1.5, -12)
barrera_entrada = box(pos=pivot_barrera_entrada + vector(0, 2, 0), size=vector(0.5, 5, 0.5), color=color.yellow)
barrera_entrada.rotation = -90  # Empieza arriba

# Barrera de salida
poste_salida = cylinder(pos=vector(7, 0.1, 12), axis=vector(0, 2.5, 0), radius=0.2, color=color.white)
pivot_barrera_salida = vector(7, 1.5, 12)
barrera_salida = box(pos=pivot_barrera_salida + vector(0, 2, 0), size=vector(0.5, 5, 0.5), color=color.orange)
barrera_salida.rotation = -90  # Empieza arriba
...
# Crear un carro estático
def crear_carro_estatico(pos, color_carro=color.blue):
    box(pos=pos, size=vector(2, 1, 4), color=color_carro)
    for dx in [-1.2, 1.2]:
        for dz in [-1.4, 1.4]:
            pos_llanta = pos + vector(dx, -0.6, dz)
            cylinder(pos=pos_llanta, axis=vector(0, 0.6, 0), radius=0.4, color=color.black)
...
def mover_carros():
    for carro in list(carros):
        ...
        if carro['estado'] == 'esperando':
            if barrera_entrada.rotation <= 0:  # La barrera está bajada (cerrada)
                obj.pos.z += 0.1
                for l in llantas:
                    l.pos.z += 0.1
...
# Movimiento de barreras
def mover_barreras():
    # Barrera de entrada
    if carros and carros[0]['estado'] == 'esperando':
        if barrera_entrada.rotation > -90:
            barrera_entrada.rotate(angle=radians(-3), axis=vector(0, 0, 1), origin=pivot_barrera_entrada)
            barrera_entrada.rotation -= 3
    elif carros and carros[0]['estado'] != 'esperando':
        if barrera_entrada.rotation < 0:
            barrera_entrada.rotate(angle=radians(3), axis=vector(0, 0, 1), origin=pivot_barrera_entrada)
            barrera_entrada.rotation += 3

    # Barrera de salida
    if carros and carros[0]['estado'] == 'salida_final':
        if barrera_salida.rotation > -90:
            barrera_salida.rotate(angle=radians(-3), axis=vector(0, 0, 1), origin=pivot_barrera_salida)
            barrera_salida.rotation -= 3
    elif not carros or (carros[0]['estado'] != 'salida_final' and barrera_salida.rotation < 0):
        if barrera_salida.rotation < 0:
            barrera_salida.rotate(angle=radians(3), axis=vector(0, 0, 1), origin=pivot_barrera_salida)
            barrera_salida.rotation += 3
